...
// Try multiple sources for ASV_bible.json (CDN → RAW → Statically)
const ASV_DATA_URLS = [
...
// Override with env for previews if needed
...
function flattenBibleJSON(input) {
  // Flat array of {book,chapter,verse,text}
  ...
  // {books:[{name,chapters:[{chapter,verses:[...] }]}]}
...
async function build() {
  ...
  const raw = await fetchJsonAny(ASV_DATA_URLS);
